        ...
        return (
            <div>
                <div className='flex justify-around flex-wrap'>
                    {movieArray.results.map((result, index) => {
                        return(
                            <div key={index} className="flex justify-center mt-20">
                            <div className="block p-6 rounded-lg shadow-lg bg-white max-w-sm">
                                <h5 className="text-gray-900 text-xl leading-tight font-medium mb-2">{result.title}</h5>
                                <p className="text-gray-700 text-base mb-4">Date de naissance</p>
                                <a href={`https://starwars.fandom.com/fr/wiki/${result.title.replace(' ', '_')}`}>
